  ...
  <div style="width: 100vw; height: 100vh">
    ...
    <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
      ...
      <van-list
        ...
        v-model="loading"
        @load="getFertilizerList"
        :offset="30"
        :finished="finished">
        <div class="fertItem" v-for="item in fertData" @click="tofertInfo(item.fertId)" :key="item.fertId">
          <div class="fertPic">
            <img v-lazy="item.fertPics[0]">
          </div>
          <div class="fertInfo">
            ...
            <div class="addfert" v-if="carList && carList[item.fertId]">
              <div class="iconReduce"
                ...
                <span class="fertNum">{{carList && carList[item.fertId].num}}</span>
              <van-icon name="add" @click.stop="addCar(item)"/>
            </div>
            <div class="fertCount" @click.stop="addCar(item)" v-else>
              <van-icon name="add" />
            </div>
          </div>
        </div>
